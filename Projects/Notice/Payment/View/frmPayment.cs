...
namespace Notice.Payment.View
{
    public partial class frmPayment : Form
    {
        ...
        {
            id = null;
            pGridEX.BindDataIntoGrid(Connection.ToDatabase(), "ListAllPayment", gridListPayment);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (id.Equals(null))
            {
                PaymentEntity.PaymentDao.SavePayment(paymentDate, items, price, des, myClass.createDate, myClass.createBy);
            }
            else
            {
                PaymentEntity.PaymentDao.SavePayment(paymentDate, items, price, des, myClass.createDate, myClass.createBy,id);
            }
            visible(true);
            Refresh();
        }
        ...
        private void gridListPayment_RowDoubleClick(object sender, Janus.Windows.GridEX.RowActionEventArgs e)
        {
            try
            {
                ...
                adapter.SelectCommand =
                    new SqlCommand("SELECT * FROM PAYMENT WHERE ID=" + gridListPayment.GetRow().Cells["ID"].Value,
