...
using Notice.Income.BankEntity;
using Pheak.GridEx;

namespace Notice.Bank.View
{
    public partial class frmBank : Form
    {
        ...
        {
            id = null;
            pGridEX.BindDataIntoGrid(Connection.ToDatabase(), "ListAllBank", gridListBank);
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (btnCashIn.Text.Equals("Cash In") && btnCashOut.Text.Equals("Cancel"))
            {
                if (id.Equals(null))
                {
                    BankDao.SaveBank(date,amount,myClass.CASH_IN,des,myClass.createDate,myClass.createBy);
                }
                else
                {
                    BankDao.SaveBank(date, amount, myClass.CASH_IN, des, myClass.createDate, myClass.createBy,id);
                }
            }
            else
            {
                if (id.Equals(null))
                {
                    BankDao.SaveBank(date, amount, myClass.CASH_OUT, des, myClass.createDate, myClass.createBy);
                }
                else
                {
                    BankDao.SaveBank(date, amount, myClass.CASH_OUT, des, myClass.createDate, myClass.createBy, id);
                }
            }
            frmIncome_Load(sender, e);
            gridListBank.Visible = true;
            btnCashIn.Text = "Cash In";
            btnCashOut.Text = "Cash Out";
        }
        ...
        private void gridListBank_RowDoubleClick(object sender, Janus.Windows.GridEX.RowActionEventArgs e)
        {
            try
            {
                ...
                dt = new DataTable();
                adapter.SelectCommand = new SqlCommand("SELECT * FROM BANK WHERE ID=" + gridListBank.GetRow().Cells["ID"].Value, Connection.ToDatabase());
                adapter.Fill(dt);
